...
### `settings.js`
Single source of truth for all tunable constants: grid/font config, density character ramp, word emergence parameters (stagger, frame skip, canvas dimensions), fluid simulation coefficients, procedural field timing, and pointer interaction forces.
...
5. **Giant letters** scale the word bitmap across the full grid with aspect-ratio correction and noise warp, blending text density into the background value
